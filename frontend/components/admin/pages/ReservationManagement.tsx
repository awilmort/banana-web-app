...
import { reservationsService, roomsService } from '@/lib/api';
...
const ReservationManagement: React.FC = () => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: t('admin.reservations.messages.updateSuccess', 'Reservation updated successfully!'), severity: 'success' });
      setEditDialog({ open: false, reservation: null });
    } catch (error: any) {
      setSnackbar({ open: true, message: error.response?.data?.message || t('admin.reservations.messages.updateFailed', 'Failed to update reservation'), severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <Container maxWidth="xl" sx={{ py: 4 }}>
        ...
        <Paper sx={{ p: 3, mb: 3 }}>
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <TextField
              size="small"
              label={t('admin.reservations.filters.search')}
              value={searchInput}
            ...
            <TextField
              size="small"
              type="date"
              label={t('admin.reservations.filters.from')}
            ...
            <TextField
              size="small"
              type="date"
              label={t('admin.reservations.filters.to')}
              value={filterDateTo}
            ...
            >
              {t('admin.reservations.filters.today')}
            </Button>
            <Button
              size="small"
              color="inherit"
            ...
            >
              {t('admin.reservations.filters.clear')}
            </Button>
            <Button
              variant="contained"
              startIcon={<Add />}
        ...
        <CreateReservationDialog
          ...
          onCreated={(newRes) => {
            ...
            setSnackbar({ open: true, message: t('admin.reservations.messages.createSuccess', 'Reservation created successfully!'), severity: 'success' });
          }}
