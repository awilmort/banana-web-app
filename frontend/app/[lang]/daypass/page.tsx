...
export default function DayPassPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await pricingService.getPricing({ service: 'daypass' });
      ...
      <Container maxWidth="md" sx={{ py: 8 }}>
        <Card sx={{ textAlign: 'center', p: 4 }}>
          ...
          <Typography variant="h4" gutterBottom color="primary">
            {t('daypass.successDaypassTitle')}
          </Typography>
          <Typography variant="body1" sx={{ mb: 3 }}>
            {t('daypass.successBody', {
              product: t('daypass.productDayPass'),
          ...
          <Button variant="outlined" onClick={() => { setSuccess(false); setBookingData(prev => ({ ...prev, visitDate: null, services: { breakfast: false, spa: false, aquaPark: true } })); }}>
            {t('daypass.bookAnotherDayPass')}
          </Button>
        </Card>
      </Container>
    );
      ...
      <Box sx={{ position: 'relative', height: { xs: '40vh', md: '50vh' }, backgroundImage: 'linear-gradient(rgba(0,0,0,0.4),rgba(0,0,0,0.4)),url(https://images.unsplash.com/photo-1544551763-46a013bb70d5?ixlib=rb-4.0.3&w=1200&q=80)', backgroundSize: 'cover', backgroundPosition: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Container maxWidth="lg">
          <Box sx={{ textAlign: 'center', color: 'white', maxWidth: 800, mx: 'auto' }}>
            <Typography variant="h2" gutterBottom sx={{ fontWeight: 700, textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>{t('daypass.heroTitle')}</Typography>
            <Typography variant="h5" gutterBottom sx={{ fontWeight: 300, textShadow: '1px 1px 2px rgba(0,0,0,0.5)' }}>{t('daypass.heroSubtitle')}</Typography>
          </Box>
        </Container>
      </Box>
              ...
              <CardContent sx={{ p: 4 }}>
                ...
                <Box sx={{ mt: 4, textAlign: 'center' }}>
                  <Button variant="contained" size="large" onClick={handleSubmit} disabled={loading || !bookingData.visitDate} sx={{ px: 6, py: 2, fontSize: '1.1rem', fontWeight: 600 }}>
                    {loading ? <><CircularProgress size={24} sx={{ mr: 2 }} />Booking...</> : t('daypass.bookButton', { price: formatMoney(calculateTotalPrice()) })}
                  </Button>
                </Box>
              </CardContent>
            </Card>
